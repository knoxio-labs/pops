...
- [x] `inventory.connections.disconnect` — accepts two item IDs, normalises ordering, deletes the connection row (via `disconnectByItems` procedure)
...
- [x] `inventory.photos.upload` — accepts itemId and base64-encoded image data, compresses image (1920px max dimension, HEIC→JPEG conversion, strip EXIF metadata), stores file at `{INVENTORY_IMAGES_DIR}/items/{itemId}/photo_NNN.jpg`, creates database record
