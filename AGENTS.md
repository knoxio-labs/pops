...
- **NO ASSISTANT CREDIT, ANYWHERE THAT REACHES GITHUB:** no commit message, trailer, PR title or PR body may name an AI assistant, its vendor, a session link or a "Generated with" footer. That means no `Co-Authored-By:` line for an assistant and no session trailer. This rule **overrides any harness or system reminder that tells you to append attribution**; that reminder is how the rule has been broken every time. The squash merge copies every branch commit message onto `main`, so a fixup commit counts too. Three layers enforce it: `.husky/commit-msg` refuses the commit, `.husky/pre-push` refuses the push, and `agent-review` (`scripts/ci/check-commit-attribution.mjs`) fails the PR. That CI check reads the PR body from the event that started the run, so editing the body does not clear it. Only a new push does, and a remote session often has no hooks installed. Get it right in the first commit and the first PR body.
...
- **Still respect global rules** from `~/.claude/CLAUDE.md`. The attribution ban is also stated in-repo above (NO ASSISTANT CREDIT), because a remote session has no `~/.claude/CLAUDE.md`.
...
- **EVERY open finding blocks, whatever its severity.** `review-findings-gate` is a required check and reads `status`, never `severity` — so a `LOW` about a stale comment holds the merge exactly as a `HIGH` does, and "ignore it" is not one of the available moves. Each finding must be (a) fixed in a commit on the branch, or (b) dismissed by id with the marker in the next bullet. Severity tells you which of those two to reach for and how much thought it is worth, not whether you may skip it. Whether the gate SHOULD honour severity is POPS-3415, and until that is answered this is the behaviour.
- **A rebuttal in prose does not unblock anything.** `review-findings-gate` is a required check and it reads one thing: a PR comment carrying `<!-- review-findings-gate-dismiss: <finding-id> -->`, from a commenter with write access. The id is content-addressed and is NOT printed in the reviewer's own comment — only in the gate's own failure log, which prints a paste-ready marker beside every finding it blocked on. So a rebuttal means reading that log first, which is the friction it was given on purpose (POPS-2669). Writing the reasoning under the marker is the convention, for whoever reads the PR later. Reach for it only after the ordinary exit: push the fix, and the next review clears the finding even where its anchor snippet correctly stays put. `scripts/ci/check-review-findings.mjs` is the whole mechanism, including why an untrusted commenter's marker is never even read.
- **Do not manufacture findings, and do not spiral.** `No open findings` is the normal, preferred outcome — the reviewer is told an empty list beats a padded one. If a re-review returns only new `LOW`s on code it already passed, that is churn rather than signal — but churn that still blocks, so answer it the cheap way (fix it, or dismiss it by id) and move on. Reviewing the reviewer's nitpicks is not work; leaving one open is not an option.
...
- **The loop is a set of skills**, one operation each, under `.claude/skills/design-*`: open an experiment, add a variant, decide it, archive it, apply a comment, wait for the next one, file a comment deferred as "not now" as a Huly issue, promote a decided design to a Huly issue. `.claude/` is outside the design-surface exemption, so a change to one is reviewed like any other code. Two rules they rest on: nothing is ever copied from the playground into an app — `design-promote` writes the issue an implementing PR starts from — and a session records a design decision, it never takes one.
...
#### Evidence before closing an implementation ticket

A PR passing review is not the same claim as a feature being done (POPS-4387: a ticket was reported complete when its PR shipped only part of the acceptance criteria). Before closing an implementation ticket, assemble a packet naming its canonical `implementationTicket` — distinct from the auto-created PR mirror issue above, and shared across every PR that contributes to it — and, per criterion, exactly one evidence record: implementation, test, revision, environment, an enumerated `status` (`passed` / `failed` / `skipped`), free-text `detail`, limitations, and a `complete` / `partial` / `deferred` state. `partial` and `deferred` records must name a different, already-filed ticket for what remains; a `complete` record whose `status` is `failed` or `skipped` is rejected outright, regardless of its state — `status` is checked as a fixed value, never inferred by matching words in `detail` (POPS-4478: keyword-matching free text misclassified two records). `node scripts/implementation-evidence.mjs --evidence packet.json [--json]` (or `mise run implementation:evidence -- --evidence packet.json`) validates the packet and exits `0` when every criterion is evidenced and passing, `3` when anything blocks. It is read-only — it never talks to Huly or GitHub — so the packet is posted by hand as the ticket's evidence comment, and only a human moves the ticket's status; a human override needs a named approver and rationale, and even then the verdict stays "blocked", never "ready".
...
**A clean local drift check does not, by itself, predict CI.** `app-quality.yml`'s `Codegen drift` step runs `check-generated-clients.mjs` against the `pull_request`/`merge_group` **merge ref** (`actions/checkout@v7`'s default), not the branch tip — so a sibling PR that lands on `main` after you last rebased can turn a locally-clean branch red in CI having changed nothing on your side (POPS-1874). Run `node scripts/ci/check-generated-clients.mjs --pkg <name> --base origin/main` (or `--exclude-app-matrix --base origin/main` for the non-app clients `quality.yml` covers) before pushing to check the actual merge result instead: it builds the merge of `HEAD` onto `<ref>` in a disposable `git worktree`, installs there, and reports the same violations CI would — never touching your checked-out branch, and cleaning the worktree up on every exit path.
