...
const ENTRY = path.join(APP_ROOT, 'dist/remote/food.js');
...
describe('food remote bundle', () => {
  ...
  // pages and the tree has twenty-four, so a top-level loop would pass while
  // every `data` tab was missing from the built bundle.
  ...
  /**
   * Every page keeps its own chunk, so a loader-mounted pillar still fetches a
   * page's code on first navigation rather than all twenty-four up front — and
   * for food that matters more than for any pillar so far, since the entry is
   * the whole cost of having it in the rail.
   *
   * One short of the slot count: the `data` index is a two-line redirect and
   * is deliberately not lazy, because a network round-trip to learn which tab
   * to open is worse than the bytes.
   */
  it('keeps every page but the redirect in its own lazily-imported chunk', async () => {
    const source = await readFile(ENTRY, 'utf8');
    const dynamicImports = [...source.matchAll(/import\("\.\/([^"]+)"\)/g)];
    expect(dynamicImports.length).toBeGreaterThanOrEqual(allPageSlots().length - 1);
  });
});
