 ...
 *
 * The interleaving tests matter here because every suite in this pillar builds
 * its app inside the test body — `makeClient(createFoodApiApp(…))` and `requestOn(app())` — so the
 * transport is handed a distinct app on nearly every call.
 */
