/**
 * A shared, pre-listened supertest transport for the food API suites.
 *
 ...
 * existed, ten sequential `conversions.listUnits()` calls produced ten
 * distinct listening ports and ten distinct client sockets; through this
 * transport the same ten produce one of each.
 *
 ...
 * lands on whatever bound the same port next.
 *
 ...
 *
 * Food builds a fresh app per call in most of its suites — `makeClient(createFoodApiApp(...))`
 * and `requestOn(app())` — so app identity travels in a header the shared
 * server routes on rather than resting on a "most recently bound app" global,
 * which interleaved calls would race.
 *
...
/** The verbs the food API suites actually issue. */
...
export function createTestTransport(): TestTransport {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (app === undefined) {
      res.statusCode = 500;
      res.end('food test transport: the request named no registered app');
      return;
    }
    app(req, res);
  });
  ...
  beforeAll(async () => {
    ...
    if (address !== '127.0.0.1') {
      throw new Error(`food test server must bind 127.0.0.1, got ${address ?? 'null'}`);
    }
  });
