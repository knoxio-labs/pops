 ...
 *
 * Projected from `@pops/ai`'s contract rather than written out a second time.
 * The pillar used to declare its nav twice — here in PascalCase and again as
 * a `NavConfigDescriptor` on the wire in kebab — with a guard making the two
 * literals agree. `navConfigFromWire` makes the second declaration
 * unnecessary, and does the projection at the type level too, so
...
export const navConfig = navConfigFromWire(AI_NAV) satisfies AppNavConfigShape;
