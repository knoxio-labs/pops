/**
 * A shared, pre-listened supertest transport for the ai API suites.
 *
 ...
 * `pillars/cerebrum/src/api/__tests__/test-http.ts` and
 * `pillars/documents/src/api/__tests__/test-http.ts`. Read finance's header
 ...
 * existed, ten sequential `aiUsage.getStats()` calls produced ten distinct
 * listening ports and ten distinct client sockets; through this transport the
 * same ten produce one of each.
 *
 * Under real machine contention that churn surfaces as a request descheduled
 * past vitest's 5s default. This pillar's sighting is
 * `ai-usage.test.ts > returns correct shape for a single entry`, which timed
 * out under the full-workspace run and passed 3/3 in isolation on a branch
 ...
 *
 * Every ai suite builds its app inside the test body — `makeClient(createAiApiApp(...))`
 * — so app identity travels in a header the shared server routes on rather
 * than resting on a "most recently bound app" global, which interleaved calls
 * would race.
 *
...
/** The verbs the ai API suites actually issue. */
...
export function createTestTransport(): TestTransport {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (app === undefined) {
      res.statusCode = 500;
      res.end('ai test transport: the request named no registered app');
      return;
    }
    app(req, res);
  });
  ...
  beforeAll(async () => {
    ...
    if (address !== '127.0.0.1') {
      throw new Error(`ai test server must bind 127.0.0.1, got ${address ?? 'null'}`);
    }
  });
