 ...
 * wrapper that resolved the id itself would hide both seams from the gate
 * behind one unresolvable call.
 *
 ...
 * restart, and the callers here are a per-URI cron loop and a per-request
 * contacts client — one line per pillar says everything a repeated one
 * would, and a per-call line would bury the leg summaries that carry the
 * counts.
 */
...
export function credentialled<TRouter>(
  ...
  try {
    ...
    if (!reportedMissingKey.has(pillarId)) {
      ...
      console.error(
        `[finance-api] cannot call '${pillarId}': no service-account key in this process. ` +
 ...
 * @param operation Dotted procedure path that was called, e.g. `entities.list`.
 */
export function credentialRejectedMessage(pillarId: string, operation: string): string {
  return (
    `[finance-api] ${pillarId} rejected this pillar's service-account credential on ` +
    `${operation} — the '${FINANCE_SERVICE_ACCOUNT_NAME}' account is missing, revoked, or not ` +
