 ...
 * reports healthy while being able to do neither. finance's own contract
 * surface — transactions, budgets, wishlist, import, corrections — is
 * entirely local and works without one; only the contacts entity fetch/match
 * and the owner-URI reconciliation cron degrade. Crashing would trade a
 * degraded reconciliation for a dead pillar. What must not happen is
 * degrading *quietly*, so the absence is reported at boot, and every leg
 ...
 */
export function configureFinanceServerSdk(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (apiKey === undefined) {
    ...
    console.error(
      `[finance-api] no service-account key: set ${SERVICE_ACCOUNT_KEY_FILE_ENV} to a mounted ` +
        ...
        'the contacts entity fetch/match and the owner-URI reconciliation cron cannot call out ' +
        'and will report no-credential every tick.'
    );
    return false;
  }
