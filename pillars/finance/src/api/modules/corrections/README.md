...
- **An AI-proposed pattern must actually occur in the description.** `ai-analyze.ts` runs the model's pattern back through `patternMatchesDescription` and returns null on a miss, so the caller falls back to a computed pattern. Without this the model invents patterns from the entity name when that name never appears in the row — "MEMBERSHIP FEE" assigned to American Express must keep `MEMBERSHIP FEE`.
