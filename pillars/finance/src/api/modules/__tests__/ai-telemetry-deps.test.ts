/**
 * The wiring gap POPS-2332 fixed: `financeTelemetryDeps()` used to leave
...
beforeEach(async () => {
  ...
  process.env['POPS_INTERNAL_CREDENTIAL'] = 'finance.a-test-secret';
});
...
describe('financeTelemetryDeps — a record the ai pillar refuses', () => {
  it('is logged naming the credential env vars, and never silently dropped', async () => {
    const { financeTelemetryDeps } = await import('../ai-telemetry-deps.js');
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => undefined);

    await financeTelemetryDeps().report?.(RECORD);
