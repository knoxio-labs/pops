...
Every AI entry is stamped with the `PROMPT_VERSION_*` of the call that produced it, and that version rides on each `source: 'ai'` suggestion through the wizard to commit. Commit writes one `ai_tag_suggestion_outcomes` row per transaction that carried an AI suggestion: the version, the AI tags offered, the tags committed (POPS-3677). A suggestion with no version — a draft processed before the stamp existed — records nothing, because it has nothing to be joined to.

`scripts/eval-tag-suggestions.ts` scores either side. With no flag it re-classifies a held-out set of reviewed spend rows with the current prompt, which calls the model; with `--outcomes <promptVersion>` it scores what live imports recorded, which does not. Both report per facet, with the false-positive rate first: Tag Review pre-accepts every suggestion, so a value on an axis the person left empty is the expensive mistake. The held-out split is `isHeldOut` in `eval-split.ts`, deterministic on the transaction id and importable from the pillar as well as the eval script — anything that shows the model prior tagging as examples must exclude those rows.
