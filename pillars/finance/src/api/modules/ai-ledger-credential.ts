/**
 * The credential finance presents to the ai pillar's usage ledger, and the
 * line it logs when a call's usage fails to land there.
 *
 * Distinct from finance's other internal-auth secrets: this is the
 * per-caller `name.secret` of ADR-039 E22, which the ai pillar verifies
 * against `POPS_INTERNAL_SECRET_FINANCE` and the `ai.usage.record` scope
 * before it will write a row. Without it a call's tokens are spent and never
 * accounted for, so fleet AI spend under-reports by exactly finance's share —
 * see POPS-2332.
 *
 * This is finance's caller identity wired onto the generic logic in
...
export const FINANCE_LEDGER_CALLER_NAME = 'finance';
...
/** The env var the ai pillar reads finance's half of the pair from. */
export const LEDGER_SECRET_ENV_AT_AI = 'POPS_INTERNAL_SECRET_FINANCE';
...
const CONFIG: LedgerCredentialConfig = {
  callerName: FINANCE_LEDGER_CALLER_NAME,
