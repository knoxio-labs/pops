...
- [x] A changed row gets entity/type/location updated plus the winning rule's tags merged in (additive-only) and match provenance stamped (`matchType: 'learned'`, `matchRuleId`, `matchConfidence`); `retroactiveReclassifications` counts only rows that actually changed; a type-only transfer/income rule updates the type/location but never clears an already-assigned entity; a `manual`-matchType row is skipped entirely.
- [x] Each rule that produced at least one change in the pass has its `timesApplied`/`lastUsedAt` bumped once, by the number of rows it changed — the same usage-bump path the explicit single-rule apply uses, aggregated across the batch scan instead of one write per row; a pass with nothing left to change bumps no rule's usage.
