...
const ENTRY = path.join(APP_ROOT, 'dist/remote/finance.js');
 ...
 */
describe('finance remote bundle', () => {
  ...
  it('carries no slot the manifest does not advertise', () => {
    const { bundles } = assertRemoteUiModule(imported);
    const advertised = FINANCE_PAGES.map((page) => page.bundleSlot).toSorted();
  ...
  it('keeps each page in its own lazily-imported chunk', async () => {
    ...
    expect(dynamicImports.length).toBeGreaterThanOrEqual(FINANCE_PAGES.length);
  });
});
