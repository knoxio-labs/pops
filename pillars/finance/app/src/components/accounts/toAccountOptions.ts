 ...
 * fetched — get the institution's name and colour already resolved.
 *
 * `logoUrl` is always left unset: `institutions.logoAssetId` names an asset
 * with no serving endpoint yet (POPS-2804, the upload flow, is unbuilt), so
 * there is nothing to resolve it to today.
 */
  ...
  return accounts.map((account) => {
    ...
    return {
      ...
      institution: institution
        ? { id: institution.id, name: institution.name, colour: institution.colour }
        : undefined,
    };
  });
}
