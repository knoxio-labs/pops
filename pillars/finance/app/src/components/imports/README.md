...
| 6   | Rules   | Offer a tag rule for each merchant whose tags nothing already covers, and stage only what is ticked. Passed over, in both directions, when there is nothing to offer.                                                                                                                                                                                                          |
...
## What the Rules step offers

Step 6 is a subtraction, not a second proposer. `computeProposals` in `rule-creation/utils.ts` does not count a tag on a row where a stored rule or the merchant's default tags supplied it, since the next import puts that tag back regardless. It also drops a tag that a rule staged on Tag Review already covers on every row the proposal came from. That comparison is by the rows the staged rule was derived from, never by pattern: Tag Review and this step derive patterns over different row sets, and comparing patterns is how one merchant's rule used to be staged twice (POPS-3674).

Nothing starts ticked. Most tags reaching the step are suggestions Tag Review accepted by default, so a pre-ticked proposal turns a guess into a rule nobody chose. The one exception is the step's own earlier choice: its staged entries carry `source: 'import-batch'`, both Create and Skip replace them, and returning to the step restores what they ticked.

When nothing is left to offer, Tag Review's Continue goes straight to Final Review and Final Review's Back comes straight back, so the step is never shown only to be skipped. Leaving that way also drops the step's earlier staged entries, since nothing proposes them any more.

A proposal that lands on an existing rule says so before it is ticked, and Final Review badges it the same way: a merge into a live rule, or a re-enable of one the user disabled, because reinforcing a rule sets it active.
