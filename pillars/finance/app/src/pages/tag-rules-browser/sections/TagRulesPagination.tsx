...
export function TagRulesPagination({
  total,
  offset,
  currentPage,
  totalPages,
  onPrevious,
  onNext,
}: TagRulesPaginationProps) {
  if (total <= PAGE_SIZE) return null;

  return (
    <div className="flex items-center justify-between">
      <p className="text-sm text-muted-foreground">
        Showing {offset + 1}–{Math.min(offset + PAGE_SIZE, total)} of {total} tag rules
      </p>
