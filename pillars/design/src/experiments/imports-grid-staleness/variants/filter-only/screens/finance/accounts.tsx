...
import { importStatusFor } from '@/fixtures/import-status';
...
const isOverdue = (account: Account): boolean =>
  !account.archived && ['due', 'stale'].includes(importStatusFor(account.id).staleness);

/**
 * Variant `filter-only`: the tiles stay as they are and the grid says nothing
...
function OverdueChip({
  ...
  return (
    <Button
      ...
      aria-pressed={on}
      onClick={onToggle}
      className={cn(on && 'border-primary bg-primary/15 text-primary hover:bg-primary/20')}
    >
      {count === 1 ? '1 account' : `${count} accounts`} overdue for an import
    </Button>
  );
}
...
export function AccountsPage({
  ...
  const visible = overdueOnly ? filters.visible.filter(isOverdue) : filters.visible;
  return (
      ...
      {accounts.length > 0 && (
        <>
          ...
          <OverdueChip
            count={overdueCount}
            on={overdueOnly}
            onToggle={() => setOverdueOnly((prev) => !prev)}
          />
          {visible.length === 0 && <NoMatchingAccounts onClear={filters.clear} />}
          {visible.length > 0 && (
