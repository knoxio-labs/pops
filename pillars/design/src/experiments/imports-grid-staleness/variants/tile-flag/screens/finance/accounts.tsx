...
/**
 * Variant `tile-flag`: an overdue account wears it on the tile, in the same
 * footer slot the archived label uses, so a glance down the grid finds the
 * one that has gone quiet without opening anything. The badge is the same
 * component the imports page shows, and it is silent on fresh accounts, so
 * the grid gains a mark only where there is something to do.
 */
