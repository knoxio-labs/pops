/**
 * Port of
 * `pillars/inventory/app/src/pages/item-form-page/sections/ConnectionsSection.tsx`.
 *
 * Distinct from `@/kit/inventory/connections` (`ConnectionsList`/
 * `ConnectDialog`), which is the item *detail* page's control for connecting
 * an item that already exists. This section only ever appears in create
 * mode, before the new item has an id to connect anything to: it collects
 * "connect to these once created" candidates inline, with its own search box
...
export interface ConnectionsSectionProps {
  ...
  searchResults: { data: ConnectCandidateItem[] } | undefined;
  searchLoading: boolean;
  onSearchChange: (value: string) => void;
  onAdd: (item: ConnectCandidateItem) => void;
...
function SearchResults({
  ...
  loading: boolean;
  filtered: ConnectCandidateItem[];
  onPick: (item: ConnectCandidateItem) => void;
}) {
  if (loading) {
    return (
...
export function ConnectionsSection({
  ...
  const filtered =
    searchResults?.data.filter((item: ConnectCandidateItem) => !pendingIds.has(item.id)) ?? [];
