/**
 * Resizable item-table columns: a grip on each header's right edge that drags
 * the column wider or narrower, answers Left and Right arrows, and fits the
 * column to its widest loaded cell on double-click or Enter.
 */
import { createContext, useCallback, useContext, useMemo, useRef, useState } from 'react';

import { cn } from '@pops/ui';

import { useEdgeDrag } from '../shared/use-edge-drag';
import { clampWidth, columnStyle, dragWidth, fitWidth, nudgeWidth } from './column-widths';
import { COLUMN } from './table-columns';

import type { KeyboardEvent, PointerEvent, ReactNode, RefObject } from 'react';

import type { ColumnId, ColumnWidths } from './column-widths';

const ColumnWidthsContext = createContext<ColumnWidths>({});

/** Supplies the set widths to every header and row cell under a table. */
export function ColumnWidthsProvider({
  widths,
  children,
}: {
  widths: ColumnWidths;
  children: ReactNode;
}) {
  return <ColumnWidthsContext.Provider value={widths}>{children}</ColumnWidthsContext.Provider>;
}

/** The width a column was given, or undefined while it keeps its default. */
export function useColumnWidth(id: ColumnId): number | undefined {
  return useContext(ColumnWidthsContext)[id];
}

/**
 * A cell's width classes: the column's own default until the column is
 * resized, then exactly the width the header was given.
 */
export function useColumnClass(id: ColumnId): string {
  const { base, sized } = COLUMN[id];
  return useColumnWidth(id) === undefined ? base : sized;
}

function measureColumn(table: HTMLElement, id: ColumnId): number[] {
  const cells = [...table.querySelectorAll<HTMLElement>(`[data-col="${id}"]`)];
  const saved = cells.map((cell) => [cell.style.width, cell.style.flex] as const);
  for (const cell of cells) {
    cell.style.width = 'max-content';
    cell.style.flex = 'none';
  }
  const widths = cells.map((cell) => cell.getBoundingClientRect().width);
  cells.forEach((cell, index) => {
    const [width, flex] = saved[index] ?? ['', ''];
    cell.style.width = width;
    cell.style.flex = flex;
  });
  return widths;
}

function currentWidth(table: HTMLElement, id: ColumnId): number {
  return table.querySelector<HTMLElement>(`[role="row"] [data-col="${id}"]`)?.offsetWidth ?? 0;
}

/** Column widths for one table, and the verbs its grips call. */
export interface ColumnWidthsApi {
  widths: ColumnWidths;
  style: Record<string, string>;
  resize: (id: ColumnId, width: number) => void;
  fit: (id: ColumnId) => void;
  widthOf: (id: ColumnId) => number;
}

/** Owns a table's column widths; `tableRef` is where cells are measured. */
export function useColumnWidths(tableRef: RefObject<HTMLElement | null>): ColumnWidthsApi {
  const [widths, setWidths] = useState<ColumnWidths>({});
  const resize = useCallback((id: ColumnId, width: number) => {
    setWidths((previous) => ({ ...previous, [id]: clampWidth(id, width) }));
  }, []);
  const fit = useCallback(
    (id: ColumnId) => {
      const table = tableRef.current;
      if (table === null) return;
      resize(id, fitWidth(id, measureColumn(table, id)));
    },
    [tableRef, resize]
  );
  const widthOf = useCallback(
    (id: ColumnId) => {
      const table = tableRef.current;
      return table === null ? 0 : currentWidth(table, id);
    },
    [tableRef]
  );
  const style = useMemo(() => columnStyle(widths), [widths]);
  return { widths, style, resize, fit, widthOf };
}

/** The grip on a column header's right edge. */
export function ColumnResizeHandle({
  id,
  label,
  api,
}: {
  id: ColumnId;
  label: string;
  api: ColumnWidthsApi;
}) {
  const width = api.widths[id];
  const startWidth = useRef(0);
  const drag = useEdgeDrag((deltaX) => api.resize(id, dragWidth(id, startWidth.current, deltaX)));
  const onPointerDown = (event: PointerEvent<HTMLDivElement>) => {
    startWidth.current = api.widthOf(id);
    drag.onPointerDown(event);
  };

  const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
    if (event.key === 'ArrowLeft' || event.key === 'ArrowRight') {
      event.preventDefault();
      event.stopPropagation();
      const direction = event.key === 'ArrowLeft' ? -1 : 1;
      api.resize(id, nudgeWidth(id, width ?? api.widthOf(id), direction));
    } else if (event.key === 'Enter') {
      event.preventDefault();
      event.stopPropagation();
      api.fit(id);
    }
  };

  return (
    <div
      role="separator"
      aria-orientation="vertical"
      aria-label={`Resize ${label}. Double-click or Enter fits the widest value.`}
      aria-valuenow={width ?? api.widthOf(id)}
      tabIndex={0}
      title="Drag to resize. Double-click to fit."
      onPointerDown={onPointerDown}
      onDoubleClick={() => api.fit(id)}
      onKeyDown={onKeyDown}
      className={cn(
        'group/grip absolute inset-y-0 -right-2 z-10 flex w-4 cursor-col-resize touch-none justify-center outline-none'
      )}
    >
      <span
        aria-hidden
        className={cn(
          'my-2 w-px rounded-full bg-border transition-colors group-hover/grip:w-0.5 group-hover/grip:bg-app-accent group-focus-visible/grip:w-0.5 group-focus-visible/grip:bg-ring',
          drag.dragging && 'w-0.5 bg-app-accent'
        )}
      />
    </div>
  );
}
