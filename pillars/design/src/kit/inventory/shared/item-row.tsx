/**
 * One row shape for items and containers alike (iOS shared row): select,
 * mark, name and quantity, placement, state badges, code, and the row's
 * verbs. Selection, focus, the optimistic pending edge and an inline
 * rejection are all states of this one row, so every list draws them alike.
 */
import { ButtonPrimitive, Checkbox, cn } from '@pops/ui';

import { CodeBadge, ContainerStateBadge, LifecycleBadge, QuantityBadge, SyncBadge } from './badges';
...
/**
 * The list wrapper rows sit in: a grid, so row selection is announced. Pass
 * `onKeyDown` and the grid takes focus and routes list keys (j, k, x) to it.
 */
