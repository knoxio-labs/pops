/**
 * The picker's rows and quick-pick sections. A row picks its place; a
 * refused row stays in view, dimmed, with its reason where the path would
 * be, so nobody has to guess why Office 04 will not take anything.
 */
...
export interface PickerRowProps {
  ...
  /** Overrides the label, for "Put back to Red toolbox". */
  label?: string;
  concept?: InventoryConcept;
  emphasis?: boolean;
  /** Tree rows drop the path: the breadcrumb above already says where they are. */
  hideDetail?: boolean;
}
...
/** A titled group of rows. Empty groups draw nothing. */
...
/** Put back, recent places, open containers and In hand: the left column. */
