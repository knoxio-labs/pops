...
function targetText(context: ExpressionContext, field: ExpressionField): string {
...
function FieldPicker({
  ...
  const fitting = resolved.ownerType.fields.filter((field) => fieldFitsSlot(field, expected));
  const options = resolved.ownerType.fields.filter(
    (field) => field.id === fieldId || fieldFitsSlot(field, expected)
  );
  return (
    <div className="space-y-2">
