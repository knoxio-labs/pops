/**
 * The measurement unit grammar and dimension math the expression-v2 mockup
 * needs: which unit symbols are known, how they combine under multiply and
 * divide, and whether two units share a dimension.
 *
 * Ported from `pillars/inventory/src/catalogue/measurement-units.ts`
 * (`inventory-types/b3-dimensional-units`, Inventory ADR-002 D5) since
 * `@pops/inventory` is off-limits to the playground. Trimmed to the pure unit
 * grammar the kit renders with; the exact rational conversion factor used by
 * the real protocol-1 codec is server-only and left out.
 */
 ...
 * `/` and the denominator's symbols. Any other text is not a unit term and
 * yields null: it is still a valid fixed unit, just one nothing here can
 * derive or convert.
 */
