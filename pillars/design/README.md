...
`src/frames/ios/primitives.tsx` holds facsimiles named after the Swift ones — `PopsCard`, `PopsRow`, `PopsButton`, `PopsActionBar`, `StateView` — so a screen designed here can be handed over in one sentence, with `state-views.tsx` (the loading/empty/error trio and `PopsStatusHeader`) and `fields.tsx` (`PopsTextField`, `PopsPhoto`, `PopsDivider`) carrying the rest. They share no code with the app and can drift; the colours and the type scale are the parts that cannot. Rasterising real SwiftUI screens beside the facsimile is POPS-2784.

The `mobile` area is the shipped app, screen for screen: transactions and one transaction, purchases, the receipt capture prompt and the three outcomes an upload has, the pairing screen, and the app shell's own states. Their copy is the app's, verbatim from its `*Copy.swift` files — a pairing screen is judged on its sentences more than on its layout, and a paraphrase here would be reviewed as the app's when it is not. Accounts is the exception and says so: `FeatureAccounts` is built but not on `RootFeature.renderable`, because BFM has no `/mobile` accounts route to bootstrap against, so those four screens are a design ahead of the app rather than a record of it.

`src/frames/ios/TabBar.tsx` follows from that. Its first three tabs are `RootFeature.renderable` in the app's order and with the app's own labels, which is why "purchases" is lower-case under a generic glyph — `RootCopy` names only Transactions and Receipts. The frame does not model the app's rule that fewer than two available features draw no bar at all; how many tabs a given server offers is a runtime answer, and `screens/mobile/shell.tsx` is where those counts are reviewed.
