/**
 * The wiring gap POPS-2332 fixed: `cerebrumTelemetryDeps()` used to leave
...
beforeEach(async () => {
  ...
  process.env['POPS_INTERNAL_CREDENTIAL'] = 'cerebrum.a-test-secret';
});
...
describe('cerebrumTelemetryDeps — a record the ai pillar refuses', () => {
  it('is logged naming the credential env vars, and never silently dropped', async () => {
    const { cerebrumTelemetryDeps } = await import('../ai-telemetry-deps.js');
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => undefined);

    await cerebrumTelemetryDeps().report?.(RECORD);
    await vi.waitFor(() => expect(warn).toHaveBeenCalled());

    const line = warn.mock.calls.map((call) => String(call[0])).join('\n');
    expect(line).toContain('403');
    expect(line).toContain('POPS_INTERNAL_CREDENTIAL_FILE');
    expect(line).toContain('POPS_INTERNAL_SECRET_CEREBRUM');
