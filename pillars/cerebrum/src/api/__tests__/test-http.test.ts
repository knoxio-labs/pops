 ...
 *   app" and the concurrency test fails: both responses come back from
 *   whichever app was bound last.
 *
 * The pooled-connection assertion is the one that matters most here. These
 * suites previously used `supertest.agent(app)`, which looks like it pools
 * and does not: its jar is for cookies, and it binds a listener and dials a
 * socket per call like bare `supertest(app)`. Reverting to it fails that
 * assertion, which is the point of stating it.
 */
...
describe('requestOn', () => {
  ...
  it('still answers correctly when two apps interleave across calls', async () => {
    ...
    expect(one.body).toEqual({ app: 'first' });
    expect(two.body).toEqual({ app: 'second' });
    expect(three.body).toEqual({ app: 'first' });
  });
