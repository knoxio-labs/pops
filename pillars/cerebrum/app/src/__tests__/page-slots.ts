import { CEREBRUM_PAGES } from '@pops/cerebrum/manifest';

/**
 * Shape of one page in `CEREBRUM_PAGES`, restated so the flatten below can walk
 ...
 *
 * `CEREBRUM_PAGES.length` counts only the top level, so a test written against it
 * would silently skip the eight `data` tabs — which is most of what POPS-3256
 * added and exactly the part most likely to break.
 */
export function allPageSlots(pages: readonly PageNode[] = CEREBRUM_PAGES): string[] {
...
export function allPages(pages: readonly PageNode[] = CEREBRUM_PAGES): PageNode[] {
  return pages.flatMap((page) => [
    page,
