/**
 * A shared, pre-listened supertest transport for the media API suites.
 *
 * Same diagnosis and same fix as `pillars/finance/src/api/__tests__/test-utils.ts`,
 * `pillars/bfm/src/api/__tests__/test-http.ts` and
 * `pillars/purchases/src/api/__tests__/test-http.ts`. Read finance's header
 ...
 * existed, ten sequential `movies.list()` calls produced ten servers, ten
 * distinct listening ports and ten distinct client sockets.
 *
 ...
 * lands on whatever bound the same port next. Media has hit both. The second
 * is diagnostic here rather than merely suspicious: this pillar's app mounts
 * no authorisation middleware and contains no 403 anywhere in its source, so
 * a 403 answering a seeding POST cannot have come from the app the test
 * built.
 *
 ...
 *
 * Media builds a fresh app per call in most of its suites, so app identity
...
/** The verbs the media API suites actually issue. */
/**
...
export function createTestTransport(): TestTransport {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (app === undefined) {
      res.statusCode = 500;
      res.end('media test transport: the request named no registered app');
      return;
    }
    app(req, res);
  });
  ...
  beforeAll(async () => {
    ...
    if (address !== '127.0.0.1') {
      throw new Error(`media test server must bind 127.0.0.1, got ${address ?? 'null'}`);
    }
  });
