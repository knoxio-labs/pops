 ...
 *
 * purchases reaches four siblings through `pillar()` from
 ...
 * readable record of what purchases calls rather than a wildcard nobody can
 * audit. Scopes match by dot prefix, so `finance.transactions` authorises
 * `finance.transactions.list` but not `finance.budgets.list`.
 *
