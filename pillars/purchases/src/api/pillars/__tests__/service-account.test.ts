...
import {
  PURCHASES_SERVICE_ACCOUNT_SCOPES,
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'purchases-service-account-'));
});
...
function keyFile(contents: string): string {
  const path = join(dir, 'pops_purchases_api_key');
...
describe('the account grant', () => {
  ...
  it('grants no whole-pillar or wildcard scope', () => {
    for (const scope of PURCHASES_SERVICE_ACCOUNT_SCOPES) {
