...
**The store is a local directory** — beside this pillar's SQLite file, and nowhere else, so `PURCHASES_SQLITE_PATH` is what points it at the volume. `createIngestClient` (POPS-2312) refuses to build a client when `PURCHASES_BASE_URL` is not a loopback address (`localhost`, `127.0.0.0/8`, `::1`), before a byte is stored or a request is sent — running the CLI against a remote host that cannot see the server's volume is a startup error now, not a silent mis-store discovered when someone tries to open an invoice. Run it where the volume is mounted.
