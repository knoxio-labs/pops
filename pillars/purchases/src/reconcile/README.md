...
A refund does not count as a claim on the total: it states what came back, never what was paid, so a refunded order is minted for just as an untouched one is. `capture`, `adjustment` and `authorization` all do count, and an order stating any of them is left alone — the minted charge is the full order total, so minting on top of one would drive the residual negative, which is a worse lie than leaving the order unexplained.
