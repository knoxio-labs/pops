...
import { nextPartId, receiptMediaType, type StagedPart } from './parts.js';

/** A chosen file that could not become a staged receipt part. */
 ...
 * chosen in without a page-count ceiling.
 *
...
export function stage(current: Staging, batch: EncodedBatch): Staging {
  ...
  return { parts: [...current.parts, ...batch.encoded], problems };
}
