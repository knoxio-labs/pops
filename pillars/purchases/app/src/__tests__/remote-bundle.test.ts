...
const ENTRY = path.join(APP_ROOT, 'dist/remote/purchases.js');
 ...
 */
describe('purchases remote bundle', () => {
  ...
  it('carries no slot the manifest does not advertise', () => {
    const { bundles } = assertRemoteUiModule(imported);
    const advertised = PURCHASES_PAGES.map((page) => page.bundleSlot).toSorted();
  ...
  it('keeps each page in its own lazily-imported chunk', async () => {
    ...
    expect(dynamicImports.length).toBeGreaterThanOrEqual(PURCHASES_PAGES.length);
  });
});
