...
- **regret detection, which the pillar cannot yet support at all** (POPS-2388). The ticket's design is line item → `pops://inventory/item/<id>` → `home_inventory.in_use = 0`, and the chain is only half built: an accepted proposal now creates the asset and writes `purchase_item_units.inventory_item_uri`, but no shipped adapter states one and no surface yet offers the accept to a human (POPS-2357), so in practice the column fills only for orders someone answers by hand. Even with the chain complete, `home_inventory.in_use` is a nullable tri-state with no date on it — NULL on every row nobody has reviewed — so `in_use = 0` cannot distinguish a thing retired after five years of use from one never taken out of its box, and a leaderboard row reading "you regret this" off it would be a confident falsehood about the most personal number in the pillar. Inventory's create body can now express the NULL (POPS-2432), and this pillar's fan-out sends no `inUse` at all, so an asset it creates lands genuinely unreviewed rather than on the regret value by default. The honest answer is that regret detection itself still waits for a signal that carries a date.
...
**`confirmedAt` is the boundary between a pass and a person**, the same idiom `purchase_item_tags` and `purchase_items.kindConfirmedAt` carry. Null means the proposal pass owns the row — it may retire the entry once no line prints that wording. Non-null means a human asserted it, and the pass may not retire, repoint or relabel it, even when the line that prompted it has been deleted. The pass runs over **every** line with no scope filter, deliberately: deriving the dictionary from a window would retire entries whose lines merely fell outside it.

**It marks a wording, so a product a human named carries its own marker.** Typing a name asserts what the thing is called, which is not a claim about any spelling of it — confirming the wordings on a rename would turn an unconfirmed merge into an asserted one nobody stated. `purchase_products.labelConfirmedAt` records the naming instead, and the pass holds back the wordings reaching a named product even while they are still proposals: retiring the last of them would orphan the product, and the orphan sweep would take the name with it. That is the one loss in this pass no re-run can undo — a retired wording comes back the moment a line prints it again, where a name only ever existed because somebody typed it. A human deliberately emptying a named product — repointing its last wording away, or deleting the entry — still removes it, because that is a person holding the tool rather than a pass reaching past one.
...
**`merchantAddressId` mirrors `merchantEntityId`'s posture, one level down.** It names the branch this purchase was made at — a `contacts` `entity_addresses` row, no FK, resolved server-side only when a reviewer already sent one — and `merchantAddressName` is the printed address kept verbatim beside it, the same way `merchantEntityName` is kept beside `merchantEntityId`. Nothing resolves a printed address string to a branch automatically; that is deliberately out of scope (ADR-053).
...
**`ordered_at` holds one spelling of an instant.** The column is TEXT and every comparison on it is lexicographic — the `from` / `to` windows, the `ORDER BY ordered_at DESC` the index and the reconcile queue read through, the equality the receipt dedup turns on. String order equals chronological order only while every row is written the same way, and `IsoTimestampSchema` admits an offset and one to nine fractional digits, so `2026-01-01T10:00:00+10:00` sorts after every `2026-01-01T0…Z` while naming an instant before them, and `…21.500Z` sorts before `…21Z`. Sydney is UTC+10/+11, so that offset is what a local caller writes by hand. `createPurchase` therefore stores `YYYY-MM-DDTHH:MM:SS.sssZ` and nothing else (`src/db/services/ordered-at.ts`), and a window bound is normalised to the same form before it becomes a predicate, so an offset bound names the window it reads as. **Two consequences on the wire:** an order posted at second precision reads back with `.000`, and a timestamp whose fields name no instant is now a refusal — at ingest rather than a row that sorts somewhere arbitrary, and on a `from` / `to` bound rather than a `200` over a window nobody asked for. Canonicalising on write rather than wrapping the SQL in `datetime()` is what keeps the linker's `(source, ordered_at)` index usable for the query it exists for, and what fixes the orderings and the in-memory folds at the same time instead of the one predicate. Migration `0010` brings rows written before this into the same form; a legacy value SQLite cannot read is left alone, which is why every fold that ranks orders still asks `order-rank.ts` for the instant rather than comparing the text — such a row is refused both ends of a group, the dictionary's printed name and the top of a hit list.
...
| `purchaseTransactionUri` | inventory's `POST /items` takes a bare `purchaseTransactionId` and has no URI field — posted as-is, the key is silently stripped by zod. Sending the id is not lossy: since POPS-1526 that pillar derives `home_inventory.purchase_transaction_uri` from it |
| `serialNumber`           | inventory has no such column anywhere (POPS-2386)                                                                                                                                                                                                           |

The last two are carried anyway: they are the strongest facts purchases holds about the asset, and a field withheld until its reader exists is a field nobody builds the reader for. The transaction is the case that argues for it — it was carried when it read as a dead end, and POPS-1526 then gave it a reader, so the id this route sends now lands in the URI column as well. The server-side route does the arithmetic, takes the bare transaction id out of the URI — and only out of a `finance`/`transaction` one, since that column means a finance transaction and splitting any URI on its last slash would file a documents id there — and puts the serial number in the row's `notes` as prose. Prose because it is visibly not a field, so nothing can mistake it for the column inventory does not have, and because the alternative is dropping the asset's strongest identity on the floor until POPS-2386 lands.
...
`deductible` is stated rather than left to inventory's default, because a default in another pillar's contract is a fact about the asset that nothing here would notice changing. It goes as `false`: purchases holds no evidence either way, and `true` would assert a claim nobody made.

`inUse` is not sent at all (POPS-2432). `home_inventory.in_use` is a nullable tri-state whose `NULL` means nobody has looked; inventory's create body can now express that by omitting the field, which is exactly the fan-out's situation — nobody has reviewed the row. Sending `false` would have meant "reviewed, not in use", a claim purchases cannot back. Every fanned-out asset now arrives genuinely unreviewed, so POPS-2388 can read `in_use = 0` as regret without excluding fan-out rows first.
...
- **`POST /search`**, declared as two adapters in the manifest — `orders` and `lineItems`, both served by the one route. The orchestrator federates a pillar when its registered manifest carries a non-empty `search.adapters`, and it reaches the pillar by POSTing `{ query, context }` over the pillar SDK. No bundle is involved, so the absent `app/` was never the reason this was missing. The order adapter matches a merchant label or order id; the line adapter matches a product name or SKU, which is the half nothing else in the fleet can answer — finance can tell you $412.80 went to Amazon, only this can tell you which order the dosing funnel was in. A line hit carries both its own line total and its owning order's total; consumers must not substitute one for the other.
