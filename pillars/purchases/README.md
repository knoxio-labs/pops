...
The payload borrows inventory's field names where that pillar has a counterpart — `itemName`, `purchaseDate`, `purchasedFromName`. **Three fields do not cross unchanged**, and a caller creating the row itself needs that before assuming a straight copy works — `src/api/inventory/asset.ts` is what the server-side route does about each:
...
The reason it is server-side rather than the browser's job is **the order those three steps happen in**. Whether a slot may still be answered is a fact in this pillar's own tables, so only a caller that can read them, create, and write the decision without yielding can guarantee the property the fan-out exists for: one accept, exactly one asset. A browser doing the same work creates first and finds out afterwards, and its remedy for a slot that was answered meanwhile is to delete a row it just made in someone else's pillar. Since a decision cannot be retracted, a duplicate asset is permanent in the way that matters — nothing in purchases points at it.
