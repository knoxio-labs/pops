...
**A caller presenting no key is still admitted, and that is a decision, not an omission.** `requireCredential` is `false`. Two callers still present none. Browser traffic is one: this pillar's SPA reaches the API through the shell's nginx proxy, which injects no key. `src/api/__tests__/two-process.test.ts` is the other, and it stays uncredentialled deliberately — it drives the real server over HTTP and is what proves the anonymous path still works, so it is the coverage for this decision rather than an instance of the problem. Closing the unauthenticated in-network path is a decision about ADR-027's docker-network boundary: it is fleet-wide, and it is not this pillar's to make alone.
...
The vocabulary is closed in the contract, and therefore in the OpenAPI projection and in every generated client, so a field outside it is a `400` from the contract itself. A supported field paired with an operator it does not take (`orderedAt eq`), or a value the field cannot hold (`status eq shipped`, a shipment status), is a `400` from the handler naming both halves. Nothing is ever dropped instead: a filter that arrives and is ignored returns a `200` no caller can tell from a filter that matched broadly, and the contract advertising a capability the handler does not have is worse than refusing the request.
