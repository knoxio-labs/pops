 ...
 *
 * The interleaving tests matter here because every orchestrator suite builds
 * its app inside the test body — `makeApp(reader(…))` per assertion, each
 * with its own snapshot reader — so the transport is handed a distinct app on
 * nearly every call.
 */
...
describe('requestOn', () => {
  ...
  it('carries app identity on every verb it exposes, not only GET', async () => {
    ...
    const responses = await Promise.all([agent.get('/who'), agent.post('/who')]);
