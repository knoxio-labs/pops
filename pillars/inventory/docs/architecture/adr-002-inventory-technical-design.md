...
The initial catalogue delivered the first six types, but every correction was a deploy. That contradicts the product's first real entry session, where a missing type and incomplete fields were ordinary owner decisions. The earlier D5 decision is replaced in full.
...
`parentTypeId` is set when a type is created and may be changed freely while the type is draft-only. Changing it on a published type is forbidden (POPS-4870).
...
| `GET /type-catalogue/protocol-rollout`          | current persisted protocol minimum and the server build's supported protocol                             |
| `POST /type-catalogue/protocol-rollout`         | compare-and-swap `{ expectedMinimumProtocol, minimumProtocol }` → the updated rollout state              |
...
Draft operations are `put_type`, `put_field`, `put_enum_option`, `archive_type`, `archive_field`, `archive_enum_option` and `reorder`; `archive_type` and `archive_field` take an optional `replacedBy` (see Archive and compatibility rules). A `put` creates when `id` is absent (the server returns its id) and updates mutable attributes when `id` is present; it cannot replace immutable attributes. For type-tree drafts, `put_type` takes `parentTypeId`. A parent and child cannot be created in one PATCH because ids are server-minted. Every response returns structured validation errors with the definition id, JSON path and code. Reads require `inventory.types.read`; all draft and publication commands require either a verified owner web session or a service account with `inventory.types.manage`. Bare in-network browser traffic is `401` on this sub-router even while D12 keeps `requireCredential: false` for the pillar's existing routes. bfm receives only the immutable read route and never the authoring surface.

#### Type tree

Types form a single-parent tree. A root is depth 1 and the maximum depth is 3. A parent is assignable to items, and filtering by a parent includes its subtypes. A subtype inherits its ancestors' fields (the same field, with the same id) and capabilities (the union of the chain). Effective fields are ordered root-first, then each type's own fields in its sort order, with the type's own fields last. Field default values come with the field. Field keys are unique among the live fields of every effective set.
...
- reusable field groups, because they add an extra concept without grouping; and
- staying flat, because it duplicates fields and `Category` cannot carry fields.
...
**A stored field may declare a default (added 2026-09-26, POPS-4844).** `defaultValues` is a list of canonical primitive wire values (the item and `set_default` encoding), `[]` for none, persisted as `item_type_fields.default_values_json`. Only a stored, non-reference field may have one (`default_not_allowed`); a `one` field holds at most one entry, and every entry must pass that field's value validation (`default_invalid`), except that an archived enum option is `default_enum_option_archived`. Both `put_field` and the whole-catalogue check that runs on every patch and publish apply these rules, so archiving an option a default names is refused until the default changes. Clients pre-fill the default on item create; the server never applies it, so an omitted field stores nothing. A default is not a value: changing one is compatible, needs no migration and re-sends no item.
...
- Forbidden: mutating or reusing an id/key, editing a published snapshot, publishing values that fail the candidate schema, changing a published type's parent (`published_type_parent_changed`) until POPS-4870, changing a published type in a way that needs a migration when it has subtypes (`migration_through_subtypes_unsupported`) until POPS-4870, or deleting audit history.

A migration is a named, version-controlled server operation from one catalogue revision to the next. It declares affected type and field ids, supports only `copy`, `set_default`, `map_enum`, `convert_decimal`, `replace_reference` and `drop_value` steps, and is dry-run against all affected rows before publication. It executes in the publication transaction, appends one ordinary item event per changed item with actor `migration`, and records counts plus the migration name on the catalogue audit event. Arbitrary SQL, JavaScript and best-effort partial rewrites are rejected. `drop_value` must be named explicitly; archive is not a euphemism for data loss. An archived field's stored values stay in the search index, so items remain findable by them, until a migration's `drop_value` step removes them.
...
**Restoring a deleted item drops an override its field no longer allows (added 2026-09-25, POPS-4354).** A migration that disables a computed field's overrides only discards the overrides live items hold (D5's compatibility rules above), so a soft-deleted item can still carry one the active catalogue has since forbidden. `item.restoreDeleted` drops any such override in the same `restored` event as the tombstone lift, keyed by the field's id, rather than reviving a value the catalogue no longer permits (`droppedOverrideChanges`/`itemRestoreDeleted` in `lifecycle.ts`); the field falls back to its computed value.

A dependency change on another item must still reach replicas holding the dependent's evaluation. `item_computed_dependencies` indexes, per dependent item, every other item its computed values read or reached (a deleted or missing reference target included). Each applied mutation, in its own transaction, refreshes the index rows of the items it changed, then re-sends every live dependent of those items: the dependent's index rows are refreshed and its `seq` is set to the mutation's last event `seq`, with no event and no revision change, so the change feed carries it beside the change with a fresh evaluation. A replica therefore accepts an item row at its stored revision when the `seq` is newer. At most 256 dependents are re-sent per mutation, lowest ids first; any beyond that keep their evaluation until they next change, which a replica shows as out of date. Publication rebuilds the whole index, and boot rebuilds it when it was built for another catalogue revision. The dependency graph counts each computed dependency uniquely per `typeId:fieldId`; a reference constrained to a type with many descendants counts once per descendant, so adding subtypes can exceed the 32-dependency cap and block publication.

A publication changes evaluations without writing the items: a computed field added, archived or restored, its expression replaced or its override policy changed, or a migration that rewrites values other items read. Both sides act on it. The server, in the publication's transaction and after the index rebuild, re-sends every live item whose effective computed definitions changed, descendants included, plus up to 256 index dependents (lowest ids first) of those items and of the items the migration wrote. Unlike the mutation path, each re-sent item gets its own `recomputed` event (no fields, its current revision, actor `migration`, not revertible) and its `seq` moves to it: a publication without a migration appends no other event, and the feed only advances past a `seq` an event holds. An item the migration wrote already carries its `migrated` event and is not re-sent again. The search index is rebuilt in the same transaction. The dependency index is per item, not per field, so a dependent is re-sent even when it reads a field the publication did not change. The phone, independently, treats a server evaluation made against an older catalogue revision than its active one as no longer current: when its active revision moves it re-evaluates every item holding an evaluation or of a type with a computed field, and shows the value as out of date where it cannot evaluate. That covers dependents beyond the cap and any publication the server did not re-send.
...
**Decision.** The phone sends no protocol header. bfm sends its own constant (`pillars/bfm/src/api/inventory/handle-factory.ts:35`). MCP (`pillars/mcp/src/tools/inventory-sync-client.ts`) and the web mutation client (`pillars/inventory/app/src/inventory-web/mutation-client.ts`) send their own constants. Inventory answers `426 client_too_old` when a received header is below `sync_meta.min_protocol`. The phone refuses a page or catalogue whose `minimumProtocol` exceeds `OnlineInventoryStore.supportedProtocol`.

“Additive fields do not bump the protocol” is false for anything bfm relays. bfm's OpenAPI objects use `additionalProperties: false`; the generated Swift decoder refuses unknown keys, and `BFMInventoryTransport` reports that as “too old”. bfm must omit an added key when it is empty: `parentTypeId` (POPS-4854) and `defaultValues` (POPS-4903).

Protocol 3 means “catalogue types may have parents”. Rollout order is:

1. inventory, bfm, MCP, web and iOS support 3 (POPS-4856, POPS-4861, POPS-4902);
2. the phone build is installed;
3. the owner activates 3;
4. then a tree can publish.

Server-to-client values that may grow (lifecycle, event kind, discard reason, repair kind) are declared as strings on the wire and decoded into Swift enums with an `.unrecognised(String)` case, as `PurchaseSettlement` does, because POPS-1663 and POPS-1992 both broke installed apps with a closed enum. Protocol and catalogue revision are independent. A label edit increments only the catalogue revision; a new primitive, provenance case or other syntax an old binary cannot safely preserve requires a new protocol. An expression node is not such syntax (D5). The persisted value-entry and definition shapes in D5 begin at protocol 2. Publication may name a higher `minimumProtocol`, but it remains a draft until an iOS build supporting that protocol is available. The owner activates it through `POST /type-catalogue/protocol-rollout`. The activation is a compare-and-swap against `expectedMinimumProtocol`, cannot exceed the server build's supported protocol and cannot decrease. Publication reads the same persisted value and refuses a catalogue minimum above it. Reversing that order strands installed offline replicas.

The server returns its current `minimumProtocol` and `catalogueRevision` on catalogue, snapshot and feed responses. An installed client that understands the protocol but lacks the named catalogue downloads that immutable snapshot before applying dependent rows. A client below the protocol minimum may keep showing its last local replica read-only, but it cannot refresh or enqueue writes.
...
- `item_types(revision, id, key COLLATE NOCASE, label, description, presentation_json, parent_type_id, sort_order, capabilities_json, legacy_labels_json, archived_at, replaced_by)`, primary key `(revision, id)` and unique `(revision, key)`; `parent_type_id` is nullable and has only `CHECK(parent_type_id <> id)`.
- `item_type_fields(revision, id, type_id, key COLLATE NOCASE, label, help, sort_order, presentation_json, kind, cardinality, required, storage, fixed_unit, reference_kinds_json, reference_type_ids_json, expression_version, expression_json, allow_override, default_values_json, archived_at, replaced_by)`, primary key `(revision, id)` and unique `(revision, type_id, key)`; `replaced_by` is null or names another definition of an archived row; `default_values_json` is a JSON array, `'[]'` when the field has no default;

The `(revision, parent_type_id)` relationship is not a SQLite foreign key: adding the composite FK requires a table rebuild, which fails with foreign keys enabled inside Drizzle's single transaction. Validation enforces parent existence and the same revision.
