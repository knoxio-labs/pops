...
- Forbidden: mutating or reusing an id/key, editing a published snapshot, publishing values that fail the candidate schema, changing a published type's parent (`published_type_parent_changed`), changing a published type in a way that needs a migration when it has subtypes (`migration_through_subtypes_unsupported`) until POPS-4870, or deleting audit history.
