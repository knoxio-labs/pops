...
import {
  INVENTORY_SERVICE_ACCOUNT_SCOPES,
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'inventory-service-account-'));
});
...
function keyFile(contents: string): string {
  const path = join(dir, 'pops_inventory_api_key');
...
describe('the account grant', () => {
  ...
  it('grants no whole-pillar or wildcard scope', () => {
    for (const scope of INVENTORY_SERVICE_ACCOUNT_SCOPES) {
