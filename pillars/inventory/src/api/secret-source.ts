 ...
 * reader — the same split `pillars/finance/src/api/secret-source.ts` and
 * `pillars/purchases/src/api/secret-source.ts` use.
 *
...
function readSecretFile(rawPath: string | undefined, source: SecretSource): string | undefined {
  ...
  try {
    ...
    console.warn(
      `[inventory] could not read ${source.fileEnvVar} (${path}): ` +
