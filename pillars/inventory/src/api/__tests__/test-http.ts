/**
 * A shared, pre-listened supertest transport for the inventory API suites.
 *
 ...
 * existed, ten sequential `items.list()` calls produced ten distinct
 ...
 *
 * Every inventory suite builds its app inside the test body, so app identity
...
/** The verbs the inventory API suites actually issue. */
...
export function createTestTransport(): TestTransport {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (app === undefined) {
      res.statusCode = 500;
      res.end('inventory test transport: the request named no registered app');
      return;
    }
    app(req, res);
  });
  ...
  beforeAll(async () => {
    ...
    if (address !== '127.0.0.1') {
      throw new Error(`inventory test server must bind 127.0.0.1, got ${address ?? 'null'}`);
    }
  });
