...
function publishRenamedDefinitions(
  ...
  const revision = created.revision.revision;
  const renamed = patchCatalogueDraft(
    harness.db,
    {
      revision,
  ...
  publishCatalogueDraft(
    ...
    {
      baseRevision: catalogue.revision,
      expectedDraftVersion: renamed.draft.revision.draftVersion,
      note: null,
    },
    AUTHOR
  );
...
function replaceField(
  ...
  const created = createCatalogueDraft(harness.db, baseRevision, AUTHOR);
  const revision = created.revision.revision;
  const replaced = patchCatalogueDraft(
    harness.db,
    { revision, baseRevision, expectedDraftVersion: created.revision.draftVersion },
    [
  ...
  publishCatalogueDraft(
    harness.db,
    revision,
    { baseRevision, expectedDraftVersion: replaced.draft.revision.draftVersion, note: null },
    AUTHOR
  );
  return revision;
}
