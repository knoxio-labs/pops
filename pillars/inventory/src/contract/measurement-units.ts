/**
 * The measurement unit model (Inventory ADR-002 D5): which unit symbols have
 * a known physical dimension, how they convert, and the unit-term grammar
 * derived units are written in (`cm²`, `cm³`, `kg/m³`).
 *
 * Every known unit converts into its dimension's coherent unit (m, kg, m³ ...)
 * by an exact power of ten, so a conversion only moves the decimal point. A
 * symbol the table does not know is its own dimension and converts only into
 * itself.
 */
 ...
 * is still a valid fixed unit, just one no arithmetic can derive or convert.
 */
...
/** The power of ten one of `term` is of its dimension's coherent unit. */
export function termPowerOfTen(term: UnitTerm): number {
  return term.reduce(
...
export interface CombinedUnit {
  ...
  /** Converts the right operand's amount into the symbols `term` kept for it. */
 ...
 * result with the same known dimension (converting the right amount into it:
 * cm × mm is cm², the mm amount divided by ten), else is appended. Symbols
 * whose powers cancel are dropped. Null when either unit is not a unit term.
 */
export function combineUnits(left: string, right: string, sign: 1 | -1): CombinedUnit | null {
  ...
  for (const factor of rightTerm) {
    ...
    if (compatible !== same)
      rightShift +=
        factor.power *
        ((KNOWN_UNITS.get(factor.symbol)?.powerOfTen ?? 0) -
          (KNOWN_UNITS.get(compatible.symbol)?.powerOfTen ?? 0));
    compatible.power += sign * factor.power;
  }
  return { term: factors.filter((factor) => factor.power !== 0), rightShift };
}

/**
 * The exact rational factor one `from` is of one `to`, as the protocol-1
 * conversion consumes it, or null when the symbols are unknown or measure
 * different dimensions.
 */
export function knownUnitFactor(
  from: string,
  to: string
): { readonly numerator: bigint; readonly denominator: bigint } | null {
  const source = KNOWN_UNITS.get(from);
  const target = KNOWN_UNITS.get(to);
  if (source === undefined || target === undefined || !sameSymbolDimension(from, to)) return null;
  const shift = source.powerOfTen - target.powerOfTen;
  return shift >= 0
