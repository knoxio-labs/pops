...
import {
  ...
  toggleField,
  togglePart,
} from '../labels/label-content.js';

import type { LabelContent, LabelDetails } from '../labels/label-content.js';
...
describe('presets', () => {
  ...
  it('finds a preset by id and nothing else', () => {
    for (const preset of LABEL_PRESETS) expect(presetById(preset.id)).toEqual(preset);
