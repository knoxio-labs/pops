...
describe('inventory remote bundle', () => {
   ...
   * the slot count: the four redirects are each a line or two and deliberately
   * eager, since a network round-trip to learn where to send the reader is
  ...
  it('splits the real pages out without giving the redirects chunks', async () => {
    ...
    expect(dynamicImports.length).toBeGreaterThanOrEqual(slots - 4);
