...
describe('unavailableSentence', () => {
  ...
  it('names a reason it does not recognise with the server code, and copes with none', () => {
    expect(
      unavailableSentence([{ reason: 'a_future_reason', fieldLabel: 'Weight', itemLabel: 'Box' }])
    ).toBe('Weight on Box is unavailable (a_future_reason).');
    expect(unavailableSentence([])).toBe('Nothing to read.');
  });
});
