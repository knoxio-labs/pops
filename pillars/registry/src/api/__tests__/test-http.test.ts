 ...
 * - drop `.agent(keepAliveAgent)` from `requestAt`'s own `route()` and the
 *   last test fails on its socket-reuse assertion. Its second assertion — the
 *   caller's `server.close()` resolving with a pooled socket outstanding —
 *   cannot be watched failing here, because Node destroys idle keep-alive
 *   connections on close; it would report as a timeout, not an assertion, if
 *   that ever stopped being true.
 *
 * The interleaving tests matter here because registry's suites build their app
 * in `beforeEach`, and `registry-dual-serve.test.ts` drives two independently
 * booted apps inside a single test — so the transport is handed a distinct app
...
import { createTestTransport, requestAt } from './test-http.js';
...
describe('requestAt', () => {
  it('pools one connection across calls to a caller-owned server, and still lets it close', async () => {
    const observed = seen();
    const server = http.createServer(recordingApp(observed));
    await once(server.listen(0, '127.0.0.1'), 'listening');
    const address = server.address();
    if (typeof address !== 'object' || address === null) {
      throw new Error('the caller-owned server reported no address');
    }
    const agent = requestAt(`http://127.0.0.1:${String(address.port)}`);

    await agent.get('/ping');
    await agent.get('/ping');

    expect(observed.remotePorts).toHaveLength(2);
    expect(new Set(observed.remotePorts).size).toBe(1);
