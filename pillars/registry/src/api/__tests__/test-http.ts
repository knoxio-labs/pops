/**
 * A shared, pre-listened supertest transport for the registry API suites.
 *
 ...
 * existed, ten sequential `GET /settings` calls produced ten distinct
 * listening ports and ten distinct client sockets; through this transport the
 * same ten produce one of each.
 *
 ...
 * lands on whatever bound the same port next. Registry's own sighting is of
 * the first kind.
 *
 ...
 *
 * Registry's suites build their app inside `beforeEach`, and the dual-serve
 * suite drives two independently-booted apps within one test, so app identity
 * travels in a header the shared server routes on rather than resting on a
 * "most recently bound app" global, which interleaved calls would race.
 *
...
/** The verbs the registry API suites actually issue. */
...
/**
 * Requests against a server the caller already runs on a real port.
 *
 * `external-pillar-e2e.test.ts` boots `createCoreApiApp` on its own listener
 * because the thing under test is the pillar SDK reaching the registry over a
 * real URL: it hands the SDK a `baseUrl` and the SDK dials it itself. That
 * server cannot be replaced by the shared one, and it is not the churn this
 * file exists to remove either — it is one listener per test, not one per
 * call.
 *
 * What is left to fix there is the client half — superagent still opens a
 * fresh connection per request — so these calls go through the same pooled
 * agent. No app header: the target owns its own port, so there is nothing to
 * dispatch between.
 *
 * Pooling is safe even though that suite closes and rebuilds its server every
 * test: since Node 19 `server.close()` destroys idle keep-alive connections
 * rather than waiting out `keepAliveTimeout`, so a pooled socket does not
 * hold the close open. `test-http.test.ts` pins both halves of that — the
 * reuse, and a close that still resolves.
 *
 * It is here rather than in the suite so the pillar keeps one door to
 * supertest, which is what `scripts/ci/check-supertest-transport-adoption.mjs`
 * enforces.
 */
export function requestAt(baseUrl: string): BoundAgent {
  const route = (test: supertest.Test): supertest.Test => test.agent(keepAliveAgent);
  return {
    get: (url) => route(supertest(baseUrl).get(url)),
    post: (url) => route(supertest(baseUrl).post(url)),
    put: (url) => route(supertest(baseUrl).put(url)),
    patch: (url) => route(supertest(baseUrl).patch(url)),
    delete: (url) => route(supertest(baseUrl).delete(url)),
  };
}
...
export function createTestTransport(): TestTransport {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (app === undefined) {
      res.statusCode = 500;
      res.end('registry test transport: the request named no registered app');
      return;
    }
    app(req, res);
  });
  ...
  beforeAll(async () => {
    ...
    if (address !== '127.0.0.1') {
      throw new Error(`registry test server must bind 127.0.0.1, got ${address ?? 'null'}`);
    }
  });
