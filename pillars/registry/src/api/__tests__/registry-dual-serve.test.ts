 ...
 * This suite proves, via the pillar's shared test transport against
 * `createCoreApiApp` on a temp SQLite, that the slash and dotted families are
 * byte-identical (modulo time-dependent fields) for register / heartbeat /
 * deregister / snapshot — so
...
describe('registry route dual-serve', () => {
  it('register: slash and dotted are byte-identical on fresh registries', async () => {
    ...
    const a = await requestOn(slash).post(REGISTRY_PATHS.register).send(registerBody());
    const b = await requestOn(dotted).post(LEGACY_REGISTRY_PATHS.register).send(registerBody());
  ...
  it('heartbeat: slash and dotted are byte-identical after registering', async () => {
    const slash = bootApp();
    const dotted = bootApp();
    await requestOn(slash).post(REGISTRY_PATHS.register).send(registerBody());
    await requestOn(dotted).post(LEGACY_REGISTRY_PATHS.register).send(registerBody());

    const a = await requestOn(slash).post(REGISTRY_PATHS.heartbeat).send({ pillarId: 'finance' });
    const b = await requestOn(dotted)
  ...
  it('heartbeat soft-fails identically (200 not-registered) for an unknown pillar', async () => {
    ...
    const a = await requestOn(slash).post(REGISTRY_PATHS.heartbeat).send({ pillarId: 'ghost' });
    const b = await requestOn(dotted)
      .post(LEGACY_REGISTRY_PATHS.heartbeat)
      .send({ pillarId: 'ghost' });
  ...
  it('deregister: slash and dotted are byte-identical after registering', async () => {
    ...
    const a = await requestOn(slash).post(REGISTRY_PATHS.deregister).send({ pillarId: 'finance' });
    const b = await requestOn(dotted)
  ...
  it('snapshot: slash and dotted are byte-identical after registering', async () => {
    ...
    const a = await requestOn(slash).get(REGISTRY_PATHS.snapshot);
    const b = await requestOn(dotted).get(LEGACY_REGISTRY_PATHS.snapshot);
  ...
  it('the SAME app serves both families against one shared registry', async () => {
    const app = bootApp();
    await requestOn(app).post(REGISTRY_PATHS.register).send(registerBody());

    const viaSlash = await requestOn(app).get(REGISTRY_PATHS.snapshot);
    const viaDotted = await requestOn(app).get(LEGACY_REGISTRY_PATHS.snapshot);
