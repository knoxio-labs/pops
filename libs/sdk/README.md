...
| `/node`            | `isCliEntrypoint` — the "is this module the process entry point" guard every pillar script needs. Node-only; see below                                                                                                                                                                                                                                                                                                                                      |
...
| `/remote-build`    | the shared-runtime contract for a pillar UI the shell mounts through its runtime loader: which packages a remote bundle must import rather than contain (`SHARED_RUNTIME_SPECIFIERS`), the Rollup `external` predicate (`isSharedRuntimeSpecifier`), and the post-build assertion that names any that got in (`findBundledSharedRuntime`). Build-time only — no runtime dependency, and nothing here is imported by a shipped bundle                        |
| `/db`              | the database safety mechanisms every pillar that owns a SQLite file composes: `withPreMigrationBackup` (snapshot before applying pending migrations), `assertDestructiveCommandAllowed` (the refusals on a wipe), and the journal readers underneath them — plus `NullableColumnKeys` and its two loops, the type-level half of writing a row payload without laundering it through a `Record<string, unknown>`. Node-only; see below                       |
| `/testing`         | test doubles for a `pillar()` consumer (stub discovery, `fakePillarHandle`) plus a real-process harness — `spawnPillarProcess`, `startRecordingProxy`, `waitForRegistration`, `resolvePillarDir` — for the handful of tests that must prove a `pillar()` call reaches a real, separately-booted peer over the wire, not a stub; and `pageTreeMismatches`, which checks a loader-mounted pillar's published page tree against the route table its app mounts |
...
`/node` is the third outlier: not about the federation, and not one pillar's private concern either. `isCliEntrypoint(moduleUrl, entryPath?)` answers "is the module at `moduleUrl` the file the process was started with", which every operator script needs so importing its `main` for a test does not also run the CLI. It used to be hand-written per script — `import.meta.url === ` a template `file://` concatenation of `process.argv[1]` never matches once the checkout path has a space, `#`, `?` or non-ASCII character in it (percent-encoding only one side), and comparing `resolve()`d paths without realpath-resolving both sides breaks the same way through a symlinked ancestor directory (macOS's `/tmp` → `/private/tmp` is enough) — two different silent-no-op failure modes from two different shortcuts. `isCliEntrypoint` resolves both, and `entryPath` is injectable so a test can assert the guard without spawning a subprocess.

`NullableColumnKeys` is there for a related reason rather than the same one: a create or update builder that passes a list of key names straight through to a row payload cannot type-check that loop against a `keyof Input & keyof Row` list, because that constraint says the key exists on both sides and nothing about the types agreeing — so the payload gets staged in a `Record<string, unknown>` and the assertion out of it hides a `NOT NULL` column being cleared or an `integer` column being fed a `boolean`. Both compiled clean in inventory (POPS-2481) and in media (POPS-3029) until the lists were constrained to the keys whose input type and column type both carry `V | null`.
...
- **`parseSoftUri`** — on the root barrel, since it is pure and framework-agnostic like `REGISTRY_PATHS`. Parses the `pops://<pillar>/<type>/<id>` grammar ([ADR-042](../../docs/architecture/adr-042-purchase-documents-and-transaction-reconciliation.md)) that every soft cross-pillar reference uses. `inventory`'s and `purchases`' nightly reconciliation crons both call it to classify a URI before probing the owning pillar.
