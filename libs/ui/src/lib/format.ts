...
/**
 * Render a minor-unit amount in its own currency, to two decimal places.
 *
 * The currency code often arrives as free text from whatever source document
 * produced the amount, and `Intl.NumberFormat` throws a `RangeError` on a code
 * it does not recognise. A whole view failing to render because one row
 * carried a typo'd code is a worse outcome than that row showing a bare
 * number, so an unknown code degrades instead of propagating.
 */
