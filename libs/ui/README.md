...
- **A menu opens on a tap, not a touch-down.** Radix's `DropdownMenu` trigger opens on `pointerdown` for every pointer and the menu is modal, so on a phone a scroll that merely started on a ⋮ button opened a menu and froze the page. `primitives/dropdown-menu.tap.tsx` makes the root controlled internally and opens touch and pen on `click`, which a scroll never fires; a mouse still opens on press.
- **Below md a `Dialog` is a full-screen sheet that scrolls itself.** `DialogContent` carries `overflow-y-auto` and safe-area padding; the page behind it is locked by Radix, so a dialog that set `overflow-hidden` on its content would put its footer back out of reach on a phone.
