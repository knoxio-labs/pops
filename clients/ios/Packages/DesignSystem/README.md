...
`LoadingStateView`, `EmptyStateView` and `ErrorStateView` take their user-facing text — `message`, and `ErrorStateView`'s `retryTitle` — from the caller, not from a string this package owns. Each falls back to a plain English default (`fallbackMessage`, `fallbackRetryTitle`) only when the caller passes nothing or a blank string, and those defaults are `String`, not `LocalizedStringKey`, on purpose.
