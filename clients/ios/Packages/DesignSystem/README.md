...
`PopsZoomablePhoto` adds pinch, pan and double-tap inspection around `PopsPhoto`. `PopsPagedPhotoViewer` pages through encoded images with a native position indicator while keeping zoom state local to each image. Both take image bytes directly and carry no receipt or purchases model, so capture and detail features can share them without importing one another.
