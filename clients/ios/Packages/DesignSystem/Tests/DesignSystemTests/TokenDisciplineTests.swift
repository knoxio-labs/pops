...
internal struct TokenDisciplineTests {
    ...
    func sourcesAreClean() throws {
        ...
        try #require(
            !files.isEmpty,
            "found no Swift sources under \(Self.sourcesRoot.path) — the scan would pass vacuously")

        let violations = try files.flatMap {
            try TokenDisciplineScanner.violations(in: $0, relativeTo: Self.packageRoot)
        }
