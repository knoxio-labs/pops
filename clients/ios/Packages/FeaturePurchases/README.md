...
`PurchasesSearchFilterFields` is `PurchasesSearchFilter`'s Show and Status pickers, plus a Tags row, as `Section` content, following `FeatureInventory`'s `InventorySearchFilterFields` idiom so a shared filter sheet can place a pillar's fields with its own header. The picker option lists live in plain functions (`purchasesSearchKindOptions()`, `purchasesSearchStatusOptions()`) rather than inline in the view, because `PurchaseSearchStatus` is not `CaseIterable` and the presentation order is worth a test independent of rendering.

`PurchasesSearchFilter.tags` is sent to the server on every ask, same as `status`; unlike `kind`, there is nothing left for the phone to narrow locally, because the wire hands back only which single field matched (`tagMatch`), never a line's full tag set. `carries(_:)` documents the any-of rule the server applies (POPS-4274) as a pure, tested function even though nothing here calls it against a live hit. The Tags row pushes `PurchasesTagPicker`, a native checklist fed by `PurchasesRepository.purchaseTags()` (POPS-3754): the BFM route orders tags most-used-first but does not put the count on the wire, so `PurchaseTagCount.count` is `nil` today and the row shows no number until it does.
