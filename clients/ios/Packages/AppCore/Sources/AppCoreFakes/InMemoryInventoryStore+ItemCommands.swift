...
extension InMemoryInventoryStore {
    ...
    private static func applyCreateProtocol2Item(
        ...
        let now = Date()
        let item = InventoryItem(
            id: new.id, revision: 1, seq: state.nextSeq, catalogueRevision: new.catalogueRevision,
            name: new.name, typeId: new.typeId, typeKey: nil,
    ...
    private static func applyEditProtocol2Item(
    ...
    private static func applyChangeProtocol2ItemType(
        id: InventoryItem.ID, catalogueRevision: Int, typeId: String,
        values: [InventoryProtocol2FieldValue], mutationId: String, into state: inout State
    ) throws {
        let item = try require(state.items[id])
        state.undoLog[mutationId] = .item(item)
        let bumpedItem = bumped(item, seq: &state.nextSeq)
        state.items[id] = protocol2Retyped(
            bumpedItem, catalogueRevision: catalogueRevision, typeId: typeId,
            fieldValues: protocol2Entries(values, revision: catalogueRevision), isContainer: false)
    }
