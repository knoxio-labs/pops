...
The scanner opens the back camera best able to focus close — the triple or dual-wide virtual camera where the phone has one, the single wide lens otherwise — because the main lens alone cannot focus nearer than about 20 cm on a Pro iPhone, and a QR framed at a natural size sits inside that. The virtual camera starts at the main lens's 1× and hands close subjects to the ultra-wide itself; a single lens is zoomed instead, by the arithmetic in `QRFocusGeometry`, the one part of the camera path a host test covers.
...
Purchase search keeps order hits and line hits distinct even when their underlying IDs coincide. Every line carries its order context, while the match text records whether the till wording or a tag caused the result. The fake searches those fields case-insensitively and applies the same settlement status vocabulary sent to the server. `search(text:status:tags:)` also takes the chosen tags; the fake records them on `searchCalls` for callers to assert against, but — like `status` — the actual any-of narrowing is the server's job (POPS-4274), because a hit's wire shape carries only the one field that matched, never a line's full tag set. `purchaseTags()` reads the tags in use, most-used first; the BFM route (POPS-3754) now puts each tag's use count on the wire (POPS-4544), so `PurchaseTagCount.count` is a plain `Int`.
