...
`UniversalSearchScreen` assembles the large title, shared search bar, scope chips, stored recents and caller-provided pillar sections. The caller supplies its domain rows, any additions to the empty-query state and its filter fields. The screen owns filter-sheet presentation and recent-query persistence; filters still apply live in their owning feature state.
