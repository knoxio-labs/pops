...
Search maps `PurchaseSearchStatus` onto the route's settlement vocabulary and omits `status` for `.any`. The wire names a merchant only by entity name, so a hit's merchant is `.printed` or `.unattributed`. A line hit's `tagMatch` is set only when `matchField` is `tag`. Blank text returns no hits without a request, because the route rejects an empty `q`. Chosen tags are sent sorted as repeated `tags` query items (POPS-4274), omitted entirely when none are chosen; `purchaseTags()` reads `GET /mobile/purchases/tags` (POPS-3754), whose response is an ordered `[String]` with no count, so every mapped `PurchaseTagCount.count` is `nil`.
