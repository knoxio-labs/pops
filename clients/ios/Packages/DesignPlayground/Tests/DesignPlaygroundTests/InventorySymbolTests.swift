...
internal struct InventorySymbolTests {
    #if canImport(UIKit)
        /// Whether the module ships the custom symbol. An Xcode build compiles
        /// the asset catalogue and the image resolves; SwiftPM's command-line
        /// build only copies the catalogue into the bundle, so there the
        /// symbol's own SVG has to be inside the copied catalogue instead.
        private func existsInModule(_ name: String) -> Bool {
            if resolvesFromModule(name) { return true }
            guard
                let catalogue = Bundle.module.url(forResource: "Symbols", withExtension: "xcassets")
            else { return false }
