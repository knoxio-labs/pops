...
/// The bug POPS-4015 was filed over: a cluster's settled unit used to be
/// whichever peer the loop visited last, which depends on dictionary
/// iteration order and is not a property of the catalogue. It has to be the
/// majority, ties broken alphabetically, every time this runs.
@Suite("Inventory tally determinism")
internal struct InventoryTallyDeterminismTests {
    private func peer(_ id: String, unit: String) -> InventoryThing {
        InventoryThing(
            id: id, name: id, symbol: "cube", location: "Bench", category: "Gadget",
            template: nil,
            properties: [InventoryProperty("Length", .measure(1, unit: unit))],
            tags: [], notes: "", suggestions: [])
    }

    @Test("the field settles on the unit most of the cluster uses")
    func majorityUnitWins() {
        let subject = peer("subject", unit: "cm")
        let catalogue = [
            subject, peer("a", unit: "cm"), peer("b", unit: "cm"), peer("c", unit: "mm"),
        ]

        let observed = InventoryObservation.observed(like: subject, in: catalogue)

        #expect(observed?.fields.first { $0.key == "Length" }?.unit == "cm")
    }

    @Test("a tie is broken alphabetically, not by enumeration order")
    func tiesBreakAlphabetically() {
        let subject = peer("subject", unit: "mm")
        let catalogueMMFirst = [subject, peer("a", unit: "mm"), peer("b", unit: "cm")]
        let catalogueCMFirst = [subject, peer("b", unit: "cm"), peer("a", unit: "mm")]

        let fromMMFirst = InventoryObservation.observed(like: subject, in: catalogueMMFirst)
        let fromCMFirst = InventoryObservation.observed(like: subject, in: catalogueCMFirst)

        #expect(fromMMFirst?.fields.first { $0.key == "Length" }?.unit == "cm")
        #expect(fromCMFirst?.fields.first { $0.key == "Length" }?.unit == "cm")
    }
}
