...
/// The staged flow's order, which the header, the progress line and the final
/// action all read from.
@Suite("Inventory create steps")
internal struct InventoryCreateStepTests {
    @Test("the steps run identity, photos, placement, review")
    func order() {
        #expect(InventoryCreateStep.allCases == [.identity, .photos, .placement, .review])
        #expect(InventoryCreateStep.identity.position == 1)
        #expect(InventoryCreateStep.review.position == 4)
    }

    @Test("only the last step is final, and it has nowhere to go next")
    func reviewIsTheEnd() {
        #expect(InventoryCreateStep.review.isFinal)
        #expect(InventoryCreateStep.review.next == nil)
        #expect(InventoryCreateStep.allCases.filter(\.isFinal).count == 1)
    }

    @Test("next and previous are each other's inverse")
    func walkingBothWays() {
        #expect(InventoryCreateStep.identity.previous == nil)
        for step in InventoryCreateStep.allCases {
