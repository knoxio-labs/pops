...
internal enum InventoryMotion {
    ...
    /// How long a staged state waits before acting, so the action's own
    /// motion plays where a reviewer can see it.
    internal static let stagedBeat = Duration.milliseconds(700)
