...
/// The month's figure on glass over the purchases wash: the month, the
/// total, the delta against the month before beside the count, and the
/// refresh status in the month's row.
internal struct PurchasesHomeFigure: View {
    ...
    /// Absent rather than zeroed when there is no earlier month: a first
    /// month drawn as a fall from nothing misstates a person's spending.
    @ViewBuilder private var deltaLine: some View {
        if let delta = digest.delta {
            Label {
                Text(
                    "\(delta.amount.formatted()) \(delta.isUp ? "more" : "less") than "
    ...
    private var count: some View {
        let noun = digest.monthCount == 1 ? "purchase" : "purchases"
        let currencies =
            digest.totals.count > 1 ? " in \(digest.totals.count) currencies" : ""
        return Text("\(digest.monthCount) \(noun)\(currencies)")
            .font(.popsSubheadline)
            .foregroundStyle(Color.popsMutedForeground)
...
/// The refresh over content already on screen, in the capsule Inventory's
/// sync status uses: nothing when current, a turning glyph while it runs,
/// and a tappable retry when it failed.
internal struct PurchasesRefreshCapsule: View {
        ...
        case .failed(let updated):
            Button(action: onRetry) {
                capsule(
                    ...
                    tone: .popsWarning, turning: false)
            }
            .buttonStyle(.plain)
    ...
    private func capsule(_ title: String, symbol: String, tone: Color, turning: Bool)
        -> some View
    {
        Label {
            Text(title)
