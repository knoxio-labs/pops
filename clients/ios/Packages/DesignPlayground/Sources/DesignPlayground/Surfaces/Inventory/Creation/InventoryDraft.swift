...
internal struct InventoryDraft: Equatable {
    ...
    internal var hasStagedWork: Bool {
        !name.trimmingCharacters(in: .whitespaces).isEmpty || code.isLabelled || !photos.isEmpty
    ...
    internal var issues: [InventoryDraftIssue] {
        ...
        if name.trimmingCharacters(in: .whitespaces).isEmpty { issues.append(.nameMissing) }
...
internal enum InventoryDraftIssue: Equatable, Identifiable {
    ...
    internal var message: String {
        switch self {
        case .nameMissing: "Give it a name. Nothing else is required."
        case .codeTaken(let existing): "\(existing) already carries that code."
        case .identifierIncomplete(let label): "The \(label) row is empty."
        case .quantityBelowOne: "A record stands for at least one thing."
        case .placementUnanswered: "Say where it goes, or put it in hand for now."
        }
    }
}
