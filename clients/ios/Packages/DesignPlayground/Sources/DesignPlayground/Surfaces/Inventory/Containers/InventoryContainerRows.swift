...
internal struct InventoryOpenContainersPanel: View {
    ...
    private func row(_ container: InventoryContainerProfile) -> some View {
        NavigationLink {
            InventoryContainerPage(profile: container)
        } label: {
            InventoryGroundedRowLabel(
                title: container.item.name,
                detail: "\(container.path) · \(container.updated)",
                symbol: "shippingbox",
...
internal struct InventoryContainerRowLabel: View {
    ...
    private var symbol: String {
        profile.isFurniture ? "cabinet" : InventorySymbol.closedContainer.system
    }
