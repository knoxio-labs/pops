...
Neither is formatted by anything. `.oxfmtrc.json` and `lint-staged.config.mjs` both exclude `clients/*/Contracts/*.openapi.json`, because a byte-equality gate against a file the pre-commit hook rewrites is a gate that fails on the first commit that touches it.
