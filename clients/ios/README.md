...
That compile builds **one architecture**, not two. Debug sets `ONLY_ACTIVE_ARCH: YES` and `build:for-testing` names the simulator it is about to test on, because against `generic/platform=iOS Simulator` Xcode forces that setting back to `NO` and compiles the tree for `arm64` _and_ `x86_64` — 66 `-target` invocations each, measured. Nothing runs the x86_64 slice: the GitHub runner image is `macos-26-arm64` and so is every machine here. `mise run build` deliberately keeps the generic destination and still compiles both, which is what keeps that a universal-compile check rather than an accident.
...
`.github/workflows/ios-quality.yml` — one job, `runs-on: macos-latest`, the only workflow in the repo that is not on Ubuntu. It selects the pinned Xcode, then runs `mise run generate:bfm-client`, `mise run lint`, `mise run build` and `mise run test ::: lint:analyze`, because a command written out a second time in a workflow file is a command that drifts. The one thing it spells out itself is the diff check after the codegen command.

The order is deliberate: `lint` needs no build and answers in seconds, so a formatting or SwiftLint violation fails before the compile rather than after it, and the compile is paid once for the test run and the analyzer rules together.
