...
- **Nothing automated exercises the Secure Enclave.** `KeychainTokenStore` is covered — `KeychainTokenStoreTests` in [`AppTests`](AppTests) runs it against a real Keychain on every CI run, because the app test target is an entitled app process. `SecureEnclaveKeyStore` cannot be: a simulator has no Enclave, so that suite is gated on `POPS_IOS_HARDWARE_TESTS=1` and closes only on a physical phone, via `mise run test:device` — which refuses to report success on a run that skipped anything. Until someone runs it, treat `SecureEnclaveKeyStore.swift` as unverified. See [`AppTests/README.md`](AppTests/README.md).
