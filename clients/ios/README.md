...
That compile builds **one architecture**, not two. Debug sets `ONLY_ACTIVE_ARCH: YES` and `build:for-testing` names the simulator it is about to test on, because against `generic/platform=iOS Simulator` Xcode forces that setting back to `NO` and compiles the tree for `arm64` _and_ `x86_64` — 66 `-target` invocations each, measured. Nothing runs the x86_64 slice: the GitHub runner image is `macos-26-arm64` and so is every machine here. `mise run build` keeps the generic destination and so still compiles both; it is a local command, not a CI step.
