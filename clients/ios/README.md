...
It builds into a **private, wiped** `clients/ios/.derived-data` rather than Xcode's shared default, and the CI job caches no derived data at all. Both follow from the same fact: a complete compiler log for `unused_declaration` — a whole-program rule — requires compiling every file, so this build has to be cold and cannot be warmed by a cache. The workflow header records what the cache did instead, which was to restore one branch's build state into another branch's job and fail it for files that branch does not contain.
