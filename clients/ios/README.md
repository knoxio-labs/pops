...
The flavour also decides **which certificate signs the build**: development locally, distribution for TestFlight. Xcode's own default is a development identity in every configuration, Release included, and that is what the first real upload archived with — so automatic signing tried to mint a development certificate on the runner, which on an ephemeral runner is a new one every run until the account reaches its certificate cap and nothing signs at all (POPS-4221). A distribution identity is cloud-managed, so `-allowProvisioningUpdates` fetches the account's existing certificate instead of creating one. `mise run verify:signing-identity` reads both flavours back out of the build settings on every PR; it compiles and signs nothing.
