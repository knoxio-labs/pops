...
vi.mock("@pops/ui", async () => {
  ...
  return {
    DataTable: ({ columns, data }: { columns: { id?: string; accessorKey?: string; header: unknown; cell: unknown }[]; data: unknown[] }) => {
      return React.createElement("table", { "data-testid": "data-table" },
        ...
        React.createElement("tbody", null,
          (data as Record<string, unknown>[]).map((row, ri) =>
            React.createElement("tr", { key: ri, "data-testid": `row-${ri}` },
              columns.map((col, ci) => {
    ...
    Button: ({ children, onClick, disabled, variant, ...rest }: Record<string, unknown>) =>
      React.createElement("button", { onClick: onClick as () => void, disabled, "data-variant": variant, ...rest }, children as React.ReactNode),
    TextInput: ({ value, onChange, placeholder, ...rest }: Record<string, unknown>) =>
      React.createElement("input", { value: value as string, onChange: onChange as () => void, placeholder: placeholder as string, ...rest }),
    Select: ({ value, onChange, options, placeholder }: { value: string; onChange: (e: { target: { value: string } }) => void; options: { value: string; label: string }[]; placeholder?: string }) =>
      React.createElement("select", { value, onChange, "aria-label": placeholder },
...
describe("RulesBrowserPage", () => {
  ...
  it("renders subtitle", () => {
    renderPage();
    expect(
      screen.getByText("Browse and manage AI categorisation rules")
    ).toBeInTheDocument();
  });
  ...
  it("shows loading skeleton", () => {
    mockListQuery.mockReturnValue({ data: null, isLoading: true, isError: false, refetch: vi.fn() });
    renderPage();
  ...
  it("filters by match type", async () => {
    ...
    // After filtering, only NETFLIX.COM (exact) should remain
    expect(screen.getByText("NETFLIX.COM")).toBeInTheDocument();
