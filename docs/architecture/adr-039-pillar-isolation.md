...
2. **Metabase — per-pillar read-only mounts.** Metabase stays a pops platform reporting concern with explicit read-only mounts of each pillar's own volume: every `pops-<pillar>-data` volume is mounted read-only into metabase at `/pillars/<pillar>` — the whole volume, so the pillar writer's live `-wal`/`-shm` keep WAL reads consistent — and metabase's SQLite datasources read `/pillars/<pillar>/<pillar>.db`. No volume is shared between two pillars, and metabase only reads. Metabase's own H2 config (`metabase.db`) rides the pops backup. This is a **documented direct-SQL BI exception** to "no cross-pillar SQL" — explicitly not an approved general pattern; new dashboards should still prefer per-pillar reporting contracts where practical.
