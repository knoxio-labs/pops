...
`check-openapi-drift.mjs` installs for a different reason than the rest of this tier: it reads no YAML or TOML at all. It needs `pnpm build` before it can run, because the committed OpenAPI spec it checks is only trustworthy once the TypeScript graph — `@pops/contract-openapi` and five more shared packages `generate-openapi.ts` imports — is compiled to `dist/`. A Tier A guard reads what is already on disk after `actions/checkout`; this guard's subject does not exist in checkable form until a build has run, which needs the workspace installed regardless of whether anything in it ever touches a parser. The tier boundary is "does this job's guard need something `actions/checkout` alone cannot provide", and a compiled `dist/` answers that the same way a YAML parser does.
