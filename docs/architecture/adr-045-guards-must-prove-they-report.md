...
| `scripts/check-exports.mjs`                      | `quality.yml` → `exports`                                 | Compiled `dist/` (`exports`/`main`/`types` targets), `package.json`                                                                                                             | none — needs a build, not a parser |
| `scripts/ci/check-openapi-drift.mjs`             | `quality.yml` → `openapi-drift`                           | Compiled `dist/` (regenerates via `generate-openapi.ts`, which imports `@pops/contract-openapi` and five more shared packages from `dist/`), committed OpenAPI JSON, `git diff` | none — needs a build, not a parser |
...
`check-exports.mjs` and `check-openapi-drift.mjs` install for a different reason than the rest of this tier: neither reads YAML or TOML. Both need `pnpm build` before they can run at all, because what they check — `exports`/`main`/`types` targets for the former, the committed OpenAPI spec for the latter — only exists once the TypeScript graph is compiled to `dist/`. A Tier A guard reads what is already on disk after `actions/checkout`; these two guards' subject does not exist until a build has run, which needs the workspace installed regardless of whether anything in it ever touches a parser. The tier boundary is "does this job's guard need something `actions/checkout` alone cannot provide", and a compiled `dist/` answers that the same way a YAML parser does.
