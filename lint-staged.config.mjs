...
export default {
  "*.{ts,tsx}": (filenames) => {
    ...
    const prettierFiles = filenames.map(esc).join(" ");

    // 1. prettier --write  (auto-fix formatting)
    // 2. eslint --fix      (auto-fix lint, per-package CWD so config resolves)
    // 3. prettier --check  (verify formatting)
    // 4. eslint            (verify lint)
    return [
      `prettier --write ${prettierFiles}`,
      ...
      ...Object.entries(groups).map(
        ([pkgDir, files]) =>
          `bash -c 'cd ${esc(pkgDir)} && eslint ${files.map(esc).join(" ")}'`
      ),
    ];
  },

  "*.css": ["prettier --write", "prettier --check"],
};
