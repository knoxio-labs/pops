...
| `ios-quality.yml`                | PR/push on `clients/ios/**`, `pillars/bfm/openapi/**`         | `macos-latest`; selects the Xcode pinned in `clients/ios/mise.toml`, then `mise run lint`, `mise run build`, and `mise run test lint:analyze` — one step, because both share a single compile |
