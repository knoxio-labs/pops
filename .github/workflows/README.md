...
| `inventory-acceptance.yml`       | `workflow_dispatch` only — **opt-in verification, not a guard** (ADR-045's 2026-09-24 amendment); not in `ci-gate.yml`, not in the ruleset | `scripts/inventory-acceptance/run.mjs` (POPS-4354) against real registry/inventory/bfm/mcp processes: vitest always, Playwright (S7) when `run_web` (default true), Maestro against a real iOS simulator (`macos-latest`) when `run_ios` (default false, tens of minutes). Writes evidence records, and packets when dispatched with `pull_request`/`pr_issue`, uploaded as artifacts |
