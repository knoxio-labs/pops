...
| `ios-testflight.yml`             | `workflow_run` of iOS Quality, acting only on a successful push run on `main`; dispatch with a `sha` on `main` | `macos-latest`, environment `testflight`; archives `Pops` and `PopsPlayground` at that commit with CalVer from `clients/ios/scripts/release-version.sh` and uploads both to TestFlight through `mise run release:testflight`. Not gated: it runs after merge |
