...
| `ios-quality.yml`                | PR/push on `clients/ios/**`, `pillars/bfm/**`, `scripts/ios-e2e/**`, `pnpm-lock.yaml`; every merge group, **scoped by a `scope` job to that same filter** | `macos-latest`; selects the Xcode pinned in `clients/ios/mise.toml`, then runs formatting and compiler-log analysis. The compiler-log analysis (`lint:analyze`, ~19.5 min of the job) runs only in the merge queue; PRs and pushes to main run the simulator tests and a Release build that verifies no BFM host is embedded instead, and post-merge pushes also run the Maestro UI flow against a real BFM. A PR whose base is not `main` (a stacked PR) skips the macOS job; the queue runs it regardless. Caches no derived data, deliberately; the header says why |
| `ios-testflight.yml`             | `workflow_run` of iOS Quality, acting only on a successful push run on `main`; dispatch with a `sha` on `main` | `macos-latest`, environment `main` (branch-restricted to `main`); archives `Pops` and `PopsPlayground` at that commit with CalVer from `clients/ios/scripts/release-version.sh` and uploads both to TestFlight through `mise run release:testflight`. Not gated: it runs after merge |
