...
| `ios-quality.yml`                | PR/push on `clients/ios/**`, `pillars/bfm/**`, `scripts/ios-e2e/**`, `pnpm-lock.yaml`; every merge group, **scoped by a `scope` job to that same filter** | `xcode-27`; selects the Xcode pinned in `clients/ios/mise.toml`, then runs formatting and compiler-log analysis. PRs additionally run the simulator tests and a Release build that verifies no BFM host is embedded; post-merge pushes also run the Maestro UI flow against a real BFM. Caches no derived data, deliberately; the header says why |
